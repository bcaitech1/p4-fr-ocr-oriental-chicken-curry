...
from networks.vim_satrn import VimSATRN
from networks.SRN import SRN
from networks.SATRN2 import SATRN2
...
def get_network(
    ...
    if model_type == "SATRN":
        model = VimSATRN(FLAGS, train_dataset, model_checkpoint).to(device)
    elif model_type == "SATRN2":
        model = SATRN2(FLAGS, train_dataset, train_group_datasets, model_checkpoint).to(device)
