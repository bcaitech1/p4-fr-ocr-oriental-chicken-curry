...
def id_to_string(tokens, data_loader,do_eval=1):
    result = []
    if do_eval:
...
def main(config_file):
    ...
    # set wandb
    wandb.run.name = options.network
    wandb.run.save()    
    ...
    for param_group in optimizer.param_groups:
        param_group["initial_lr"] = options.optimizer.max_lr
    if options.optimizer.scheduler == 'cycle':
    ...
    elif options.optimizer.scheduler == 'cosine':
