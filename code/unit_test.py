...
#     h, w = d['image'].squeeze(0).squeeze(0).shape
#     if h * 0.7 > w and w > h * 0.5:
#         image_np = cv2.rotate(d['image'].squeeze(0).squeeze(0).detach().cpu().numpy(), cv2.ROTATE_90_COUNTERCLOCKWISE)
#         # print(d['path'][0].split('/')[-1])
#         cv2.imwrite(f"./ckimg_70/{d['path'][0].split('/')[-1]}", image_np)
#         cnt += 1
...
# img = cv2.imread('/opt/ml/input/data/eval_dataset/images/train_00029.jpg', cv2.IMREAD_GRAYSCALE)
# w, h = img.shape
