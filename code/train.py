...
def main(config_file):
    ...
    if options.optimizer.is_cycle:
        ...
        lr_scheduler = CosineAnnealingWarmUpRestarts(
            optimizer, 
            T_0= len(train_data_loader) * 40,
            T_mult=1, 
            eta_max=5E-4,
            T_up= len(train_data_loader) * 5, 
            gamma= 0.3)
    ...
    for epoch in range(options.num_epochs):
        ...
        train_result = run_epoch(
            ...
            optimizer,
            lr_scheduler,
            options.teacher_forcing_ratio - epoch *  0.3 / options.num_epochs,
        ...
        if epoch % options.print_epochs == 0 or epoch == options.num_epochs - 1:
            ...
            wandb.log({
                ...
                'val/loss' : validation_result["loss"],
                ...
                'val/socre' : validation_epoch_sentence_accuracy* 0.9 + (1 - validation_epoch_wer)*0.1,
                'teacher_forcing' : options.teacher_forcing_ratio - epoch *  0.3 / options.num_epochs
            })


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    #wandb.init(project='OCR', name='Attention_test_GRU')
    ...
    # wandb init
    wandb.init(project=parser.wandb_project,name='Attention_test_2', config=hparams)
