...
import math
import random
from networks.swin_transformer_2d import SwinTransformer
...
class TransformerEncoderFor2DFeatures(nn.Module):
    ...
    def __init__(
        ...
        window_size = (input_size[0]//32, input_size[1]//32)
        self.shallow_cnn = SwinTransformer(
            pretrain_img_size=input_size, window_size=window_size,
...
class SWIN_SATRN(nn.Module):
    def __init__(self, FLAGS, train_dataset, checkpoint=None):
        super(SWIN_SATRN, self).__init__()

        self.encoder = TransformerEncoderFor2DFeatures(
            input_size=(FLAGS.input_size.height, FLAGS.input_size.width), 
            input_channel=FLAGS.data.rgb,
            depths=FLAGS.SWIN_SATRN.encoder.depths,
            num_heads=FLAGS.SWIN_SATRN.encoder.num_heads,
            hidden_dim=FLAGS.SWIN_SATRN.encoder.hidden_dim,
            filter_size=FLAGS.SWIN_SATRN.encoder.filter_dim,
            head_num=FLAGS.SWIN_SATRN.encoder.head_num,
            layer_num=FLAGS.SWIN_SATRN.encoder.layer_num,
            dropout_rate=FLAGS.dropout_rate,
        )

        self.decoder = TransformerDecoder(
            num_classes=len(train_dataset.id_to_token),
            src_dim=FLAGS.SWIN_SATRN.decoder.src_dim,
            hidden_dim=FLAGS.SWIN_SATRN.decoder.hidden_dim,
            filter_dim=FLAGS.SWIN_SATRN.decoder.filter_dim,
            head_num=FLAGS.SWIN_SATRN.decoder.head_num,
            dropout_rate=FLAGS.dropout_rate,
            pad_id=train_dataset.token_to_id[PAD],
            st_id=train_dataset.token_to_id[START],
            layer_num=FLAGS.SWIN_SATRN.decoder.layer_num,
        )
