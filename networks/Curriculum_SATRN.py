...
class SWSLResNextBackbone300(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.coordconv = CoordConvNet()
        self.net = timm.create_model('swsl_resnext101_32x16d', in_chans=32, features_only=True, pretrained=True)
        del self.net['layer4']
        del self.net['layer3']
...
class TransformerEncoderFor2DFeatures(nn.Module):
    ...
    def __init__(
        ...
        elif backbone == "swsl_resnext":
            self.shallow_cnn = SWSLResNextBackbone300() # SWSL ResNext사용
        else:
            self.shallow_cnn = DeepCNN300( # denseNet 기반
            input_size,
            num_in_features=48,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, tgt_prev, src, tgt_mask):
        ...
        if tgt_prev == None:  # Train (teacher forcing하는 경우  이 케이스로 들어가서 한방에 끝내고 teacher forcing 안하는 경우는 처음 1회 수행때만 수행을 하게 됨)
            ...
            att = self.attention_layer(out, src, src) # q 는 내꺼, k,v는 encoder에서 넘어온 것(src)
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        # 추가 작업
        self.generator1 = nn.Linear(hidden_dim,group_num_classes[0])
        self.generator2 = nn.Linear(hidden_dim, num_classes)

        self.pad_id = pad_id
        self.st_id = st_id

        self.group_pad_id1 = group_pad_id[0]
        self.group_st_id1 = group_st_id[0]
    ...
    def group_pad_mask1(self, text):
        pad_mask = text == self.group_pad_id1
        pad_mask[:, 0] = False
        pad_mask = pad_mask.unsqueeze(1)
    ...
    def forward(
        self, src, text, is_train=True, batch_max_length=50, teacher_forcing_ratio=1.0, group_stat = False
    ):
        ...
        if is_train and random.random() < teacher_forcing_ratio: # teacher forcing부분
            ...
            if group_stat == 0:
                tgt_mask = self.group_pad_mask1(text) | self.order_mask(text.size(1))
            elif group_stat == 1:
                tgt_mask = self.pad_mask(text) | self.order_mask(text.size(1)) # order mask : 뒷쪽을 보지 않음, pad_mask : pad 토큰을 고려하지 않음
            
            for layer in self.attention_layers:
                tgt = layer(tgt, None, src, tgt_mask)

            if group_stat == 0:
                out = self.generator1(tgt)  # [b, 1, c]
            elif group_stat == 1:
                out = self.generator2(tgt)  # [b, 1, c]
            ...
            # 추가 부분
            if group_stat == 0:
                target = torch.LongTensor(src.size(0)).fill_(self.group_st_id1).to(device) # [START] token
            elif group_stat == 1:
            ...
            for t in range(num_steps):
                ...
                # 추가 작업
                if group_stat == 0:
                    _out = self.generator1(tgt)  # [b, 1, c]
                elif group_stat == 1:
                    _out = self.generator2(tgt)  # [b, 1, c]
...
class SATRN2(nn.Module):
    def __init__(
        self, 
        FLAGS, 
        train_dataset,
        train_group_datasets, 
        checkpoint=None
        ):
        super(SATRN2, self).__init__()

        group_num_classes = []
        group_pad_ids = []
        group_st_ids = []
        for i in range(len(train_group_datasets)):
            group_num_classes.append(len(train_group_datasets[i].id_to_token))
            group_pad_ids.append(train_group_datasets[i].token_to_id[PAD])
            group_st_ids.append(train_group_datasets[i].token_to_id[START])
        ...
        self.decoder = TransformerDecoder(
            ...
            # group_num_classes = len(train_group_datasets[0].id_to_token),
            ...
            group_pad_id = group_pad_ids,
            group_st_id = group_st_ids,
    ...
    def forward(self, input, expected, is_train, teacher_forcing_ratio, group_stat):
